    ...
    _factorY ( 824 )
{
    ...
    addMeeples( 1, std::map< uint16_t, uint8_t >{ {1, 39}, {2, 34}, {3, 32} } );
    //.at( i )->setScale( scalePixmapMeeple );
...
void PlayboardScene::addMeeples( uint8_t color, std::map< uint16_t, uint8_t > tagsPoint2Ds )
{
    ...
    {
        _mapPlayerToMeepleItems[ color ][ it->first ] =
            new QGraphicsPixmapItem( *(_mapPixmapMeeples[ color ]) );
        _groupMeeples->addToGroup( _mapPlayerToMeepleItems[ color ][ it->first ] );
        _mapPlayerToMeepleItems[ color ][ it->first ]->setScale( _scalePixmapMeeple );
